...
import { useAdminUpdateUserMutation, useDeleteUserDataMutation } from "../../Slices/adminApiSlice";
...
export const UsersTable = ({ users, refetchData }) => {
  ...
  const { isVisible: showRegisterModal, openModal: openRegisterModal, closeModal: closeRegisterModal } = useModal();
  const { isVisible: showUpdateModal, openModal: openUpdateModal, closeModal: closeUpdateModal } = useModal();
  ...
  const handleDelete = async (user) => {
    try {
      await deleteUserByAdmin({ userId: user._id });
  ...
  const handleUpdate = async () => {
    if (!userNameToUpdate || !userEmailToUpdate) {
  ...
  return (
    <>
      ...
      <Button className="dark-button" onClick={openRegisterModal}>
        Add User
      </Button>
        ...
        <MDBTableBody>
          {filteredUsers.map((item, index) => (
            <tr key={index}>
              ...
              <td>
                <Button variant="transparent" size="sm" onClick={() => handleOpenUpdateModal(item)}>
                  <AiFillEdit />
                </Button>
              </td>
              <td>
                <Button variant="transparent" size="sm" onClick={() => handleDelete(item)}>
                  <AiFillDelete />
                </Button>
              </td>
            </tr>
      ...
      {showRegisterModal && (
        <RegisterModal showModal={showRegisterModal} closeModal={closeRegisterModal} refetchData={refetchData} />
      )}
    </>
  );
};
